"""The layer between what the model computed and what a person reads.

Two properties matter more than the rest. Every factor the engine fits must be
mapped, or it silently disappears from the evidence while still moving the
score. And the prose must be built from each name's own values -- a template
with the ticker swapped reads as understanding and conveys nothing, which on a
screen whose output is a position someone may take is worse than saying less.
"""
...
from prosignal.presentation.evidence import (
    EVIDENCE_CATEGORIES, FACTOR_MAP, build_evidence, confirmation_count,
)
...
# ------------------------------------------------------------- completeness
def test_every_factor_the_engine_fits_is_mapped():
    """An unmapped factor vanishes from the evidence while still moving the
    score, which is the exact failure this layer exists to prevent."""
    from prosignal.features import crosssec

    source = Path(crosssec.__file__).read_text(encoding="utf-8")
    fitted = {name for name in FACTOR_MAP}
    ...
    assert not missing, f"factors not mapped to a category: {missing}"
...
# ----------------------------------------------------------------- verdicts
def test_a_low_risk_reading_is_favourable_not_weak():
    """Every risk factor is good when LOW. Orienting them naively would call
    the safest name in the market its worst."""
    cats = {c.key: c for c in build_evidence({
        "downside_vol": factor(-1.5), "max_dd_120": factor(-1.4),
        "beta_120": factor(-1.2), "amihud": factor(-1.1), "max5_21": factor(-1.0),
    })}
    assert cats["risk"].verdict == "Contained"


def test_high_volatility_reads_as_elevated_risk():
    cats = {c.key: c for c in build_evidence({
        "downside_vol": factor(1.8), "max_dd_120": factor(1.6),
    })}
    assert cats["risk"].verdict == "Elevated"


def test_a_name_near_the_average_is_called_neutral():
    cats = {c.key: c for c in build_evidence({"resid_mom": factor(0.1)})}
    assert cats["momentum"].verdict == "Neutral"


def test_the_category_is_weighted_by_how_much_each_factor_moves_the_score():
    """An equal average lets a factor the model barely uses outvote the one
    carrying the block."""
    cats = {c.key: c for c in build_evidence({
...
def test_an_unavailable_factor_does_not_count_as_evidence():
    cats = {c.key: c for c in build_evidence({
        "earnings_yield": factor(2.0, available=False),
    })}
...
def test_confirmation_counts_only_categories_that_could_be_judged():
    cats = build_evidence({"resid_mom": factor(1.5), "prox_52w": factor(1.4)})
    agree, judged = confirmation_count(cats)
    assert judged == 2 and agree == 2
...
def test_two_different_names_do_not_produce_the_same_paragraph():
    a = _card("AAA", 1, {"resid_mom": factor(2.0), "downside_vol": factor(-1.5)})
    b = _card("BBB", 2, {"earnings_yield": factor(2.0), "downside_vol": factor(1.8)})
...
def test_the_thesis_names_the_area_that_actually_leads():
    card = _card("AAA", 1, {"earnings_yield": factor(2.4), "resid_mom": factor(0.05)})
    n = build_narrative(card, build_evidence(card["factors"]), status="BUY",
                        entry_rank=8, exit_rank=16)
    assert "Earnings yield" in n["thesis"]


def test_a_watch_name_is_told_how_far_it_has_to_climb():
    card = _card("AAA", 11, {"resid_mom": factor(1.2)})
...
def test_a_buy_name_is_told_what_would_close_it():
    card = _card("AAA", 2, {"resid_mom": factor(1.2)})
    n = build_narrative(card, build_evidence(card["factors"]), status="BUY",
                        entry_rank=8, exit_rank=16)
...
def test_the_penalty_is_shown_as_a_reason_not_as_a_score_delta():
    ...
    card = _card("AAA", 1, {"resid_mom": factor(1.2)}, against=[
...
def test_no_engine_vocabulary_reaches_the_prose():
    card = _card("AAA", 1, {
        "resid_mom": factor(1.5), "deliv_pct": factor(1.2),
        "prox_52w": factor(1.1), "amihud": factor(-0.9),
    })
    n = build_narrative(card, build_evidence(card["factors"]), status="BUY",
                        entry_rank=8, exit_rank=16)
    blob = json.dumps(n)
...
def test_the_technical_detail_survives_behind_the_card():
    ...
    card = _card("AAA", 1, {"resid_mom": factor(1.5)})
    card["why"] = ["resid_mom: +1.55 sd ... (coefficient +0.02036)"]
    card["score"] = 0.9
