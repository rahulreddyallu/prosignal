...
def test_the_live_refit_applies_the_admission_predicate():
    """The gap this file was written to close.

    The R9 correction was threaded into `research_panel` -- the RESEARCH path
    -- and `fit_predict`, which is what the engine refits with, never received
    it. So the audited numbers moved and the shipped coefficients did not,
    which is a worse state than either: the measurement and the model disagree
    ...
    assert "admission_rules" in inspect.signature(cm.fit_predict).parameters

    fit_src = inspect.getsource(cm.fit_predict)
    assert "admission_rules=admission_rules" in fit_src, (
        "`fit_predict` accepts the predicate and does not pass it to the panel"
    )

    stage_src = inspect.getsource(s4._cross_sectional_model)
    assert "train_on_admissible_only" in stage_src, (
        "the live refit does not read the policy, so the switch is a config "
        "value nothing acts on -- which is R12, one finding along"
    )


def test_the_live_refit_asks_the_cache_for_the_admissible_population(
        live_cfg, tmp_path, monkeypatch):
    """The behavioural half. The test above reads source; this one runs it.

    `load_cached` is the first thing stage 4 does with the fingerprint, so
    capturing its `label` argument shows what population the live path
    believes it is fitting -- without paying for a fit. A source assertion
    alone survives someone setting `admission_rules = None` two lines earlier,
    which is exactly the mutation that got past it.
    """
    from prosignal.features import crossmodel as cm
    from prosignal.stages import stage4_core_score as s4
    ...
    def _spy(path, as_of, refit_every=None, estimator=None, label=None):
        seen["label"] = label
        raise _Stop

    monkeypatch.setattr(cm, "load_cached", _spy)

    class _Store:
        curated = tmp_path

        def price_sessions(self):
            return [d.date() for d in pd.bdate_range("2020-01-01", periods=900)]
    ...
    p = live_cfg.params
    try:
        s4._cross_sectional_model(_Store(), ["AAA"], dt.date(2023, 6, 1),
                                  p.stage4_core_score, p.universe,
                                  risk_cfg=p.stage7_risk)
    except _Stop:
        pass

    assert seen.get("label"), "stage 4 never consulted the cache"
    assert seen["label"].get("population") == "admissible", (
        f"the live refit is asking for a fit on the "
        f"{seen['label'].get('population')!r} population while the config says "
        f"train_on_admissible_only; the switch is read by nothing"
    )
    assert "admission" in seen["label"], (
        "the population is named but the predicate that defines it is not, so "
        "two different invalidation geometries share one cache entry"
    )
