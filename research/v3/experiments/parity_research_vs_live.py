...
from prosignal.features import v3, v3_factors                    # noqa: E402
...
def research_score(store, as_of, symbols, sectors, sessions):
    ...
    try:
        ...
        fund = None
    # THE BENCHMARK IS THE MARKET AS IT STOOD -- the equal-weight mean of the
    # names in hand, which is how both `build_v3_panel` and `build_v3_block`
    # build it. `resid_rev_21` is the only factor that reads it.
    #
    # Omitting it here cost a diagnosis: `fund` landed in the `bench_ret`
    # positional slot, so the fundamentals block was silently empty and
    # resid_rev_21 came back all-NaN -- and the harness reported the RESEARCH
    # AND LIVE PATHS AS DIVERGING at rho 0.91 when the paths agreed to
    # 0.000e+00 on every factor that was actually computed. A parity harness
    # that fails for its own reasons is worse than no parity harness.
    raw = v3_factors.factor_frame(
        close, open_, vwap, turnover, deliv,
        (close / close.shift(1) - 1.0).mean(axis=1), fund)
...
def main() -> int:
    ...
    themes = v3.THEMES
    print(f"one scorer: {len(themes)} themes, {len(v3.ALL_FACTORS)} factors")
