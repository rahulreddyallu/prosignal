...
def cmd_data_manifest(cfg: AppConfig, args: argparse.Namespace) -> int:
    ...
    if getattr(args, "verify", False):
        ...
        for d in drift[: int(getattr(args, "limit", 20) or 20)]:
            # `Drift` carries `kind` and `detail`; an earlier version read `.what`,
            # which does not exist -- so the verify path raised AttributeError
            # exactly when it had drift to report, and reported nothing whenever
            # it mattered. A reporting path that only runs on the unhappy branch
            # has to be exercised on that branch to be known to work.
            _detail = f" ({d.detail})" if getattr(d, "detail", "") else ""
            _print(f"    {d.path}: {d.kind}{_detail}")
        if len(drift) > 20:
